...
def get_user_by_auth_id(auth_user_id: str) -> Optional[Dict[str, Any]]:
    """get user by auth id. Returns None if not found (avoids .single() raising on 0 rows)."""
    db = get_db()
    try:
        response = db.table('users').select('*').eq('auth_user_id', auth_user_id).limit(1).execute()
...
def get_conversation(conversation_id: str) -> Optional[Dict[str, Any]]:
    ...
    db = get_db()
    response = db.table('conversations').select('*').eq('id', conversation_id).single().execute()
    return response.data
...
def list_conversations(user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    response = (db.table('conversations')
                .select('id, title, conversation_mode, created_at, updated_at, thumbnail_url')
                .eq('user_id', user_id)
...
def update_conversation(conversation_id: str, **fields) -> Optional[Dict[str, Any]]:
    """update conversation fields"""
    from datetime import datetime, timezone
    db = get_db()
    fields['updated_at'] = datetime.now(timezone.utc).isoformat()
    response = db.table('conversations').update(fields).eq('id', conversation_id).execute()
