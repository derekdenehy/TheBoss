...
class TestLoginOnce:
    ...
    def test_login_once_complete_execution(self, mock_playwright, mock_input):
        ...
        # Import and execute the script (it runs on import)
        # Remove from cache if already imported
        if 'scraper.login_once' in sys.modules:
            del sys.modules['scraper.login_once']
        
        # Now import with patches in place
        from scraper import login_once
...
if __name__ == '__main__':
    pytest.main([__file__, '-v', '--cov=scraper.login_once', '--cov-report=term'])
